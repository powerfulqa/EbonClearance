...
- Run `stylua *.lua && luacheck *.lua` before committing. Luacheck sits at **0 warnings** (cleaned post-v2.6.0); keep it at zero. If a new warning appears, fix the cause or extend [`.luacheckrc`](.luacheckrc) - do not silence with blanket directives. **Luacheck now runs in CI** (`luacheck EbonClearance_*.lua` in [.github/workflows/test.yml](.github/workflows/test.yml)), so a new warning fails the build, not just review.
