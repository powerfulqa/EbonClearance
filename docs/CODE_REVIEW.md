...
| 8b | Extract `EbonClearance_Minimap.lua` (minimap button + LDB launcher + combat-vendor SecureActionButton, ~210 LOC). Three self-contained UI buttons outside the Interface Options panel hierarchy. Uses `_G[<panel-name>]` lookup for MainOptions / ProcessBagsPanel instead of explicit NS exposures | **DONE** (commit `<pending>`) |
| 8c | Future: extract remaining UI surface (Interface Options panels, CreateListUI + list-row factories, tooltip annotation, bag context menu). Pacing TBD - this is the densest cross-file dependency cluster in the addon, deserves careful subdivision | pending |
