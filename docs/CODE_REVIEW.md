...
| 8e-ii | Future: extract remaining UI surface (other Interface Options panels: Merchant, Character, Whitelist family, Blacklist family, Profiles, ImportExport, Deletion, Scavenger, Main; CreateListUI + list-row factories; panel infrastructure helpers). Subdivide further by panel groups to keep each stage reviewable | pending |
