...
- **Default ON as of v2.59.1** (was opt-in default OFF in v2.58.0). A single `shareServerData` flag controls share + view (it shows no names, so there's no name-sharing toggle). `shareServerDataDefaultBumped` is the one-shot migration marker: EnsureDB sets it either on fresh-install seeding or on the first v2.59.1 login for a character that had the v2.58.0 default-off value stored. Once set, EnsureDB never touches `shareServerData` again - the user can flip it off in the panel and it stays off. The channel is joined **only** when it's on (login flow, staggered 7s after the version probe) - opted-out users pay zero channel-slot cost. **Realm-wide update alerts have NO separate toggle:** the version carried in `SREQ`/`SDAT` feeds the existing `versionAlerts` nudge (via `NotePeerVersion`), so a stats-sharer's normal update alert also covers the realm. This deliberately avoids forcing the always-on `versionAlerts` to auto-join a channel for every user.
