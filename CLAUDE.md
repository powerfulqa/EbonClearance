...
- This is a WoW 3.3.5a addon for Project Ebonhold. After the file split (docs/CODE_REVIEW.md item 4), the v2.36.0 Help / Stats sub-panel extractions, the v2.38.0 Quickstart panel, the v2.39.0 `EbonClearance_Comms.lua` addition, the v2.40.0 guild-share panel (`EbonClearance_GuildShare.lua` + `EbonClearance_GuildPanel.lua`), the v2.43.0 localization layer (`EbonClearance_Locale.lua` + the per-language `EbonClearance_Locale_frFR.lua` / `EbonClearance_Locale_deDE.lua` templates, loaded right after Core), and the v2.53.0 chance-on-hit proc-pairing share (`EbonClearance_ProcShare.lua`, another `NS.Comms` consumer sharing `ADB.chanceProcConfirmedItems` across guild peers), and the v2.57.0 interactive Sold History window (`EbonClearance_HistoryWindow.lua`, an on-demand scrollable window over the full-session sell/delete logs, modelled on the Loot Log window), the addon ships as 34 `.lua` files; the event hub + slash commands + Bindings.xml glue live in [EbonClearance_Events.lua](EbonClearance_Events.lua) (renamed from the original monolith `EbonClearance.lua` in Stage 9). Addon-to-addon comms (the version-update gossip + the reusable `NS.Comms` transport) live in [EbonClearance_Comms.lua](EbonClearance_Comms.lua), which loads after the event hub; `EbonClearance_GuildShare.lua` + `EbonClearance_ProcShare.lua` (both `NS.Comms` consumers) + `EbonClearance_GuildPanel.lua` add the guild/group-scoped, opt-in, anonymous-by-default sharing ("Stats - Guild" + Chance-on-Hit knowledge). The [.toc](EbonClearance.toc) lists every file in load order.
- No external libraries. All Blizzard APIs.
- Run `stylua *.lua && luacheck *.lua` before committing. If a new warning appears, fix the cause or extend [`.luacheckrc`](.luacheckrc) - do not silence with blanket directives. **Luacheck CI gating is ENABLED** (the pre-existing debt was reconciled): [.github/workflows/test.yml](.github/workflows/test.yml) installs luacheck via luarocks and runs `luacheck EbonClearance_*.lua` as a gate on every push, alongside `luac -p` and all eight suites. The gate must stay at **0 warnings** - the WoW 3.3.5a API surface is declared in `read_globals`, intentional globals (`SetItemRef`, the loot-log handler/binding globals, `GameTooltip`/`ItemRefTooltip` field writes) are declared, and idiomatic-noise categories (`212` unused args, `213`, `431`/`432` shadowing, `631` long lines) are ignored with rationale in `.luacheckrc`. Anything else: fix the cause or add the global/API to `.luacheckrc`, never a blanket silence.
- Run all eight invariant tests before committing. The single entry point is **`lua tests/run_all.lua`** (runs every suite below and is the source of truth for the suite list; CI and the release gate both call it). To run one suite in isolation, invoke it directly:
  ...
  - `lua tests/test_perf_guardrails.lua` - the v2.24.0 perf invariants (BAG_UPDATE coalescing, name-sort pre-compute, search debounce, affix-data cache by itemString) plus v2.29.0 invariants (normaliseAffixDesc case-fold, EnsureAccountDB allowedAffixes migration, list-mutation refresh call sites, sell-border helpers pinned to EC_compCache), plus the v2.55.0 frame-spike diagnostic invariants (Test 115: session-only bounded ring, delta-based watchdog gated on per-frame elapsed, each phase timed additively via debugprofilestop, tooltip body kept separate from its timing wrapper, `/ec spike` wired to `NS.ShowFrameSpikes`).
  ...
  - `lua tests/test_procshare.lua` - the v2.53.0 proc-share invariants. Loads `EbonClearance_ProcShare.lua` in isolation and unit-tests the pair encode/decode (`id~spell~family~item` records) + local-wins merge policy (attempted overwrites of existing `ADB.chanceProcConfirmedItems` entries are rejected; only new itemIDs are written) + delimiter-unsafe field skipping + payload cap (240 bytes; oldest `learnedAt` trimmed first), plus transport handler tests (reply gated on `shareChanceProcs`, self-skip, sender name never stored in ADB or the recent-merges ring), plus static-pattern checks (uses `NS.Comms`, registers `"PREQ"`/`"PDAT"`, no `RegisterAddonMessagePrefix`, no `GROUP_ROSTER_UPDATE`).
  ...
  - `lua tests/test_affix_resilience.lua` - the v2.56.0 affix-source resilience invariants. Static-pattern checks that every FUNCTIONAL read of PE's learned-affix catalog routes through the single accessor `EC_compCache.getExtractionCatalog()` (Protection.lua) rather than the raw `_G.ExtractionService` global; that the accessor honours the `simulateExtractionAbsent` switch (returns nil); that Protection.lua has exactly one non-comment raw read (inside the accessor); that the `refreshKnownAffixes` spellbook walk is the unconditional primary source with the ExtractionService merge as an additive `if catalog then` step; and that `/ec affixfallback` exists. Diagnostics (`/ec captureproc`, `/ec procdump`, `/ec bugreport`) intentionally keep reading the raw global to report true live state.
...
8. The workflow runs (~1-2 min) and re-runs all eight test suites at the gate. The version-bump bot commit (`Update version to vX.Y.Z [skip ci]`) lands on `origin/master`, carrying the CI-side rewrites of `EbonClearance.toc` + `EbonClearance_Events.lua`'s `ADDON_VERSION`.
