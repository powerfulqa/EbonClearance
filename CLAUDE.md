...
- Run all seven invariant tests before committing. The single entry point is **`lua tests/run_all.lua`** (runs every suite below and is the source of truth for the suite list; CI and the release gate both call it). To run one suite in isolation, invoke it directly:
