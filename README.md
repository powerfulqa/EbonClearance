...
- **Statistics.** Lifetime + session counters side-by-side, reset independently. Includes per-rarity breakdowns of sells and deletions, top-5 most-sold and most-deleted items, lifetime Process Bags counters (Disenchant / Mill / Prospect / Pick Lock), and a top-zones leaderboard by lifetime gold earned at vendor. **Stats - Guild** pools opted-in guild/group members anonymously; **Stats - Server** (opt-in, off by default) is a realm-wide "collective odometer" showing the combined, anonymous totals of everyone online and sharing right now (gold vendored, items sold/deleted/processed, busiest zones, most-sold items, live sharer count) - not a leaderboard, never names. It rides a hidden chat channel (one channel slot while switched on) and can also carry realm-wide update alerts.
...
| `/ec loot` | Open the Loot Log window (Session / Character / Account views; also main-panel and Stats-panel buttons and a bindable key) |
